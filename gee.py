...
def parseStmtList(  ):
	""" gee = { Statement } """
	tok = tokens.peek( )
	while tok is not None: # Want print w respect to AST
...
def parse( text ) :
	...
	tokens = Lexer( text )
	expr = addExpr( ) # Notice
	print (str(expr)) # Comment these two later and uncom remaining
	#     Or:
